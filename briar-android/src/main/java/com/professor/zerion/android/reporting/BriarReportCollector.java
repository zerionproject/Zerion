...
import com.professor.zerion.android.reporting.ReportData.MultiReportInfo;
import com.professor.zerion.android.reporting.ReportData.ReportItem;
import com.professor.zerion.android.reporting.ReportData.SingleReportInfo;
import com.professor.zerion.android.api.MemoryStats;
import com.professor.zerion.android.api.NetworkUsageMetrics;
import com.professor.zerion.android.api.NetworkUsageMetrics.Metrics;
...
class BriarReportCollector {
	...
	ReportData collectReportData(@Nullable Throwable t, long appStartTime,
		...
		reportData.add(getTimeInfo(appStartTime));
		if (memoryStats != null) {
			reportData.add(getMemory(memoryStats));
		}
		return reportData
				.add(getStorage())
				.add(getConnectivity())
	...
	private ReportItem getMemory(MemoryStats stats) {
		...
		// Memory stats simplified - only two methods available
		memInfo.add("MemoryUsed", stats.getMemoryUsed());
		memInfo.add("MemoryTotal", stats.getMemoryTotal());
