...
The application depends on these libraries published by the Briar Project (GPLv3 unless stated): `tor-android` and `lyrebird-android` (packaged Tor and pluggable-transport binaries), `jtorctl` (BSD), `dont-kill-me-lib`, `socks-socket`, `null-safety`.
