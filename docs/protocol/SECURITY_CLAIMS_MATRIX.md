...
| 6 | Fixed 4096-byte frames at a paced, jittered, cover-filled cadence **within a live connection**: a real frame and a cover frame are the same size, so an observer of an established connection cannot distinguish message from cover or infer size/timing **within a regime**. The cadence has two constant regimes: an active slot interval while application records flowed recently or are queued, and a slower idle interval afterwards (slower still on metered networks unless the user disables the reduction). Each regime is constant-rate with zero-mean jitter and a frame is never sent closer than the active spacing, so activity onset cannot burst; what an observer of an established connection can learn from the regime transitions is the coarse onset and end of activity, not sizes, counts or which frames carried data. Pacing is self-paced (a stall lengthens the cadence, no catch-up bursts); connection existence, lifetime, reconnects and session count are not hidden, and global traffic confirmation against Tor is out of scope. | `ZppSendScheduler.tick()` emits exactly one fixed-size ZWF frame per iteration; `ZppConnectionRunnerImpl.computeInterval` zero-mean jitter; `ZppPacingPolicy` two-regime intervals; `SlotClock.awaitNextSlot` active-spacing floor; `FRAME_LENGTH` padding in the encrypter. | `ZppSendSchedulerTest.everyTickSendsExactlyOneFrame`; `ZppPacingRegimeTest`; jitter/framing tests |
