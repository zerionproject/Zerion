...
Pairing starts out of band. There are two pairing paths, and in 3.0.11 they differ:
...
1. Both sides exchange hybrid public keys (X25519 + ML-KEM-768). The master key combines a static-static X25519 agreement, an ephemeral-ephemeral X25519 agreement and an ML-KEM-768 encapsulation to the peer's *ephemeral* ML-KEM key, through a keyed BLAKE2b KDF with domain separation. The pairing secret is therefore confidential against a future quantum adversary.
2. Authentication is classical: each side proves ownership of the master key with a MAC, the result is bound to the out-of-band commitment (so a man-in-the-middle who relays the handshake cannot match it), the contact record exchanged afterwards is signed with Ed25519, and the B.3 proof binds the static ML-KEM key to the Ed25519 identity. The ML-DSA-65 half of the identity does not participate in pairing, so pairing has **no post-quantum authentication in 3.0.11**; a quantum adversary active at pairing time could impersonate a peer. Post-quantum authentication of the pairing (a mandatory hybrid Ed25519 + ML-DSA-65 signature over the exchange, handshake minor version 3) is implemented on the `security-r1` branch and ships in 3.0.12, which is not yet released; this section will be updated with that release. No impersonation is known to have occurred; it would require an adversary with a cryptographically relevant quantum computer active during the pairing.
3. The handshake is **downgrade-resistant**: once a contact is paired with the hybrid protocol, a later attempt that offers only the classical protocol is rejected.

**Nearby pairing (QR code or Bluetooth key agreement).** In 3.0.11 this path is a QR key-agreement protocol (BQP version 4) with a classical X25519 key agreement bound to the QR commitment; the resulting contact is stored as classical, and its transport root key is classical. Its application frames still receive the per-message ML-KEM-768 mix of [§6](#6-the-mode-3-full-ratchet), but the stream identifiers, frame headers before the first ML-KEM contribution and cover bodies of a nearby-paired contact are not post-quantum protected. A post-quantum nearby pairing (BQP version 5, one ML-KEM-768 encapsulation) is implemented on the `security-r1` branch and ships in 3.0.12, which is not yet released.
...
- **Voice and video calls** are peer-to-peer over Tor: Opus audio (16 kHz mono, ~24 kbit/s) in fixed 20 ms / 640-byte frames and H.264 video in padded frames, designed to be encrypted with AES-256-GCM under a call key negotiated over the authenticated messaging channel. The fixed audio frame size and cadence avoid leaking speech patterns through packet timing. **Status in 3.0.11:** the derivation of the per-call keys has a defect (the derived key material is zeroed before use) and the video nonce can repeat when video is stopped and restarted within one call; both are corrected on the `security-r1` branch and ship in 3.0.12, which is not yet released (`SECURITY.md`). Until then, call confidentiality and integrity rest on the Tor onion-service layer between the two devices, not on the application-layer cipher. There is no indication that the defect was exploited; using it would require an adversary inside the Tor connection or at one of the two devices.
...
- **Mutual authentication**: classical authentication at pairing (ownership MACs bound to the out-of-band commitment, Ed25519 contact-record signature and B.3 proof) and per-frame AEAD thereafter; the post-quantum public key and ciphertext are authenticated before use.
...
- **Pairing authentication is classical in 3.0.11** ([§4](#4-identity-and-pairing-key-exchange)), and nearby pairing is classical throughout. Post-quantum protection at pairing is confidentiality (harvest-now-decrypt-later resistance), not authentication.
...
- **Call media** in 3.0.11 is protected by the Tor onion-service layer only ([§8](#8-11-chat-group-chat-channels-and-voice)).
...
| Voice | Opus + AES-256-GCM (design; see §8 for the 3.0.11 status) | 16 kHz mono ~24 kbit/s, 20 ms / 640-byte frames |
