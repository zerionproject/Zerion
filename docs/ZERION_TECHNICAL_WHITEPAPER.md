...
- **Voice calls** are peer-to-peer over Tor: uncompressed 16 kHz mono PCM (256 kbit/s) in fixed 20 ms / 640-byte frames, each frame encrypted with AES-256-GCM under a call key negotiated over the authenticated messaging channel. The fixed frame size and cadence avoid leaking speech patterns through packet timing, and no codec-dependent frame sizes exist to leak them either.
...
| Voice | PCM + AES-256-GCM | 16 kHz mono 256 kbit/s, 20 ms / 640-byte frames |
...
*Zerion is built on the Briar Project's Bramble framework and is licensed under the GPLv3. The native transport (ZTP/ZWF/ZPP/ZMM) and the Mode 3-Full ratchet described here are Zerion's own work; Briar is credited for the identity, storage and Tor foundations on which they build.*
